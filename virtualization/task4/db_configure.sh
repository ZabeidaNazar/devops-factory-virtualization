...
sudo apt-get update
sudo apt-get install mysql-server prometheus-node-exporter -y
...
if [[ -n "$MYSQL_USERNAME" && -n "$MYSQL_PASSWORD" ]]; then
    sudo mysql -e "CREATE USER IF NOT EXISTS '$MYSQL_USERNAME'@'%' IDENTIFIED WITH caching_sha2_password BY '$MYSQL_PASSWORD';"
    sudo mysql -e "GRANT SELECT ON test_db.users TO '$MYSQL_USERNAME'@'%';"
